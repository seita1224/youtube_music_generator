...
backend 側 (``ymg_backend.infrastructure.storage``) と同等のインターフェースを
提供し、 GPU worker が ``output_uri`` (``file:// / s3:// / gs://``) へ生成物を
書き出せるようにする。 fsspec の ``AbstractFileSystem`` を内包し、 protocol に
応じたバックエンドへ委譲する。
...
class StorageAdapter:
    ...
    Args:
        base_uri: 相対パス解決の基点となる URI (例: ``file:///srv/ymg/outputs/``)。
    ...
    def __init__(
        ...
        *,
        ...
        resolved_base = (
            base_uri if base_uri is not None else os.environ.get(_STORAGE_BASE_URI_ENV)
        )
